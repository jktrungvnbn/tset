...
public class ServiceAdmin {
    ...
    public ModelNhanVien getNV(int idNV) throws SQLException {
        ...
        String sql = "SELECT ID_NV,TenNV, DATE_FORMAT(NgayVL, '%d-%m-%Y')DH NgayVL,SDT,ChucVu,ID_NQL,TinhTrang FROM NhanVien WHERE ID_NV=?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, idNV);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    public ArrayList<ModelHoaDon> getListHDIn(String txt) throws SQLException {
        ...
        String sql = "SELECT ID_HoaDon,ID_KH,ID_Ban,DATE_FORMAT(NgayHD,'dd-mm-YYYY')HD as Ngay,Tienmonan,Tiengiam,Tongtien FROM HoaDon";
        if (txt.equals("Tất cả")) {
            sql = "SELECT ID_HoaDon,ID_KH,ID_Ban,DATE_FORMAT(NgayHD,'dd-mm-YYYY')HD as Ngay,Tienmonan,Tiengiam,Tongtien FROM HoaDon";
